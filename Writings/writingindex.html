// Securing the passphrase using environment variables
...
export default function SecurePDFUploadApp() {
  ...
  // Password now lives in env file as NEXT_PUBLIC_SECRET_PASS
  const handleUnlock = () => {
    const CORRECT_PASSWORD = process.env.NEXT_PUBLIC_SECRET_PASS;
    if (inputPassword === CORRECT_PASSWORD) {
      setIsUnlocked(true);
    } else {
      alert('No wings yet, try again.');
    }
  };
